...
WHERE CAST(t.amount AS FLOAT64) BETWEEN 100 AND 200
...
-- Ejercicio 2
CREATE OR REPLACE TABLE `sprint3-analytics-borja-b.sprint3_golden.product_sales_ranking` AS
SELECT
p.product_id,
p.name,
REPLACE(SAFE_CAST(p.price AS STRING),'.',',') AS price,
p.colour,
